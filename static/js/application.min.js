...
(function(){jetbrick.api.ScrollToTop=(function(){function a(c,b){var d=this;this.dom=c;this.options=$.extend({},jetbrick.api.ScrollToTop.defaults,this.dom.data(),b);if(this.dom.is("body")){this.dom=$("<img src='"+this.options.image+"' title='回到顶部' />");$(document.body).append(this.dom);}this.dom.css({display:"none",position:"fixed",cursor:"pointer",right:this.options["position-right"],bottom:this.options["position-bottom"]});this.show=false;this.dom.on("click",function(){d.scrollup();return false;});$(window).bind("scroll resize",function(){return d.toggleHandler();});}a.prototype.scrollup=function(){var b,c;c=0;if(this.options.dest){b=$(this.options.dest);c=b.length===0?0:b.offset().top;}return $(document.body).animate({scrollTop:c},this.options.duration);};a.prototype.toggleHandler=function(){var b;b=$(window).scrollTop()>=this.options.from;if(b&&!this.show){this.dom.stop().fadeIn();this.show=true;}if(this.show&&!b){this.dom.stop().fadeOut();return this.show=false;}};return a;})();jetbrick.api.ScrollToTop.defaults={from:30,dest:null,"position-right":20,"position-bottom":20,duration:80,image:"scroll-to-top.png"};}).call(this);
